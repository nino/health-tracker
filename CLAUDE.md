...
- Native verification (from `app/`):
  ...
  ```
- Device builds go through EAS (`eas.json`: `development` = dev client, `preview` = sideloadable APK for Android, `production` = App Store / TestFlight). EAS is logged in as ninoan (`~/.bun/bin/eas`); ask before production/submit actions.
- TestFlight (verified working 2026-09-05): from `app/`, `eas build --platform ios --profile production --auto-submit` builds on EAS servers and uploads to App Store Connect; the build then shows up under TestFlight after processing. Build numbers come from EAS (`appVersionSource: remote`), so never edit `buildNumber` by hand — only bump `version` in `app.json` for a new marketing version. Credentials and the App Store Connect record are EAS-managed.
...
- `src/catalog/` — the heart of the app: a data-driven catalog of all 39 HealthKit symptom category types (name, emoji icon, `hkIdentifier`, a `ValueKind` — severity / presence / appetite — supplying picker options and section title) plus the three metrics (mood/stress/anxiety with their ranges). Everything else (main-screen grid, settings toggles, log sheets, charts) derives from it. User-defined items (`custom_items` table, entry kind `"custom:<uuid>"`; severity, 1–10 rating, event, numeric, or free text) flow through the same surfaces via the adapters in `src/catalog/custom.ts`; they never mirror to a health backend, deletion is archival only, and an item's kind is fixed after creation. Events (value always 1), numeric items (any typed number, decimals allowed — SQLite's column affinity stores them losslessly despite the INTEGER declaration), text items (per-topic notes: value 0, text in `entries.value_text`), and the built-in quick-note tile (kind `"note"`) stay out of next-up; events chart as weekly counts, numeric items as lines with a data-derived y-domain (the one non-fixed domain — see `numericDomain`), text items list their entries in history, quick notes not at all.
