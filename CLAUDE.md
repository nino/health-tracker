...
- `ContentView.swift` — main grid (Mood/Stress/Anxiety + enabled symptoms + Random), owns the last-logged dates state, refreshes it when any sheet dismisses, re-renders every minute via `TimelineView` so relative ages/staleness colors stay current. All log sheets present through one `LogTarget` enum (metric or symptom) so "Save & Next" can swap the sheet in place to the least-recently-logged other tile; the swapped-in view gets an explicit `.id` (state would otherwise leak between targets), and symptom recency is patched in-memory during a chain because the HealthKit reload only runs on final dismiss. Symptom recency comes from HealthKit; mood/stress/anxiety recency from the local store (deliberately NOT HealthKit — cold-start State of Mind queries were part of the launch lag, and it misses nothing unless mood was logged from outside this app).
